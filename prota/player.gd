...
@export var footstep_distance := 60.0 
...
func _ready():
	...
	if Global.checkpointPos != Vector2(-999, -999):
...
func _update_animation():
	...
	match current_state:
		...
		PlayerState.MOVING:
			if isRunning: _stateMachine.travel("run")
			else: _stateMachine.travel("walk")
...
var deathMenuScene: PackedScene = preload("res://scenes/death_menu.tscn")
func _die():
	...
	await get_tree().create_timer(1).timeout
	whiteout()
	fade_out_audio("Master", 2.0, -40.0)
	is_dead = true
	await get_tree().create_timer(1).timeout
...
func openDeathMenu():
	var deathMenu = deathMenuScene.instantiate()
	
	var layer = CanvasLayer.new()
	layer.layer = 20
	layer.process_mode = Node.PROCESS_MODE_ALWAYS
	get_tree().current_scene.add_child(layer)
	layer.add_child(deathMenu)
	
	deathMenu.process_mode = Node.PROCESS_MODE_ALWAYS
	deathMenu.open()
...
func fade_out_whiteout(duration := 0.7) -> void:
	var layers = get_tree().get_nodes_in_group("whiteout_layer")
	for layer in layers:
		if not is_instance_valid(layer):
			continue
		var rect = layer.get_child(0) as ColorRect
		var tween = create_tween()
		tween.tween_property(rect, "color", Color(1,1,1,0), duration)
